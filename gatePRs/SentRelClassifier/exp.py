...
import logging
from XSNLPReader.readerPostProcessor import SentRelPostProcessor
...
if __name__ == "__main__":
    ...
    parser.add_argument("--x_fields", help="x fileds", default='tweet_text')
    parser.add_argument("--y_field", help="y filed", default='tweet_label')
    ...
    readerPostProcessor = SentRelPostProcessor(args.query_field, x_fields, args.y_field, config=config)
    ...
    if args.trainInput: 
        ...
        print(train_dataIter.target_labels)
        #print(next(train_dataIter))
        ...
        dummy_config = {'MODEL':{'n_classes':len(train_dataIter.target_labels), 'sep_idx':readerPostProcessor.max_query_len+1}}
        if 'MODEL' in config:
        ...
        if args.splitValidation:
            ...
            mm.genPreBuildModel('BERT_Rel_Simple')
            mm.train(train_dataIter, save_path=args.savePath, valDataIter=test_dataIter, earlyStopping=True, patience=3, batch_size=args.batch_size, warm_up=5, earlyStoppingFunction=train_loss_early_stopping, num_epoches=args.num_epoches)
